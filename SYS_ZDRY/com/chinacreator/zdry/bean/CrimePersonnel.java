package com.chinacreator.zdry.bean;

import com.chinacreator.common.bean.QueryBean;
import com.chinacreator.common.constant.CommonConstant;
import com.chinacreator.common.helper.DictionaryCacheHellper;

/**
 * @author 禹文超
 * @mail wenchao.yu@chinacreator.com
 * @Description: TODO(犯罪人员实体)
 * @date Mar 4, 2013 5:19:05 PM
 */
public class CrimePersonnel extends QueryBean {

	/**
	 * 
	 */
	private static final long serialVersionUID = -4046416436012409005L;

	private String xb;
	private String mz;
	private String zzmm;
	private String wwcd;
	private String zy;
	private String hjdqh;
	private String hjdxz;
	private String sspcsdm;
	private String sspcsmc;
	private String ajmc;
	private String ajlb;
	private String jyaq;
	private String jdsj;
	private String sxzm;
	private String bzrxm;
	private String bzj;
	private String fkje;
	private String barxm;
	private String badwdm;
	private String badwmc;
	private String sjly;
	private String rksj;
	private String isrl;
	private String rlrxm;
	private String rldwdm;
	private String rldwmc;
	private String bh;
	private String xm;
	private String sfzh;
	private String csrq;
	private String jdjg;
	private String jdjgdm;
	private String rylx;
	
	private String endJdsj;
	
	private String startJdsj;
	private String xbmc;
	
	@SuppressWarnings("unused")
	private String cz;

	public String getXb() {
		return xb;
	}

	public void setXb(String xb) {
		this.xb = xb;
	}

	public String getMz() {
		return mz;
	}

	public void setMz(String mz) {
		this.mz = mz;
	}

	public String getZzmm() {
		return zzmm;
	}

	public void setZzmm(String zzmm) {
		this.zzmm = zzmm;
	}

	public String getWwcd() {
		return wwcd;
	}

	public void setWwcd(String wwcd) {
		this.wwcd = wwcd;
	}

	public String getZy() {
		return zy;
	}

	public void setZy(String zy) {
		this.zy = zy;
	}

	public String getHjdqh() {
		return hjdqh;
	}

	public void setHjdqh(String hjdqh) {
		this.hjdqh = hjdqh;
	}

	public String getHjdxz() {
		return hjdxz;
	}

	public void setHjdxz(String hjdxz) {
		this.hjdxz = hjdxz;
	}

	public String getSspcsdm() {
		return sspcsdm;
	}

	public void setSspcsdm(String sspcsdm) {
		this.sspcsdm = sspcsdm;
	}

	public String getSspcsmc() {
		return sspcsmc;
	}

	public void setSspcsmc(String sspcsmc) {
		this.sspcsmc = sspcsmc;
	}

	public String getAjmc() {
		return ajmc;
	}

	public void setAjmc(String ajmc) {
		this.ajmc = ajmc;
	}

	public String getAjlb() {
		return ajlb;
	}

	public void setAjlb(String ajlb) {
		this.ajlb = ajlb;
	}

	public String getJyaq() {
		return jyaq;
	}

	public void setJyaq(String jyaq) {
		this.jyaq = jyaq;
	}

	public String getJdsj() {
		return jdsj;
	}

	public void setJdsj(String jdsj) {
		this.jdsj = jdsj;
	}

	public String getSxzm() {
		return sxzm;
	}

	public void setSxzm(String sxzm) {
		this.sxzm = sxzm;
	}

	public String getBzrxm() {
		return bzrxm;
	}

	public void setBzrxm(String bzrxm) {
		this.bzrxm = bzrxm;
	}

	public String getBzj() {
		return bzj;
	}

	public void setBzj(String bzj) {
		this.bzj = bzj;
	}

	public String getFkje() {
		return fkje;
	}

	public void setFkje(String fkje) {
		this.fkje = fkje;
	}

	public String getBarxm() {
		return barxm;
	}

	public void setBarxm(String barxm) {
		this.barxm = barxm;
	}

	public String getBadwdm() {
		return badwdm;
	}

	public void setBadwdm(String badwdm) {
		this.badwdm = badwdm;
	}

	public String getBadwmc() {
		return badwmc;
	}

	public void setBadwmc(String badwmc) {
		this.badwmc = badwmc;
	}

	public String getSjly() {
		return sjly;
	}

	public void setSjly(String sjly) {
		this.sjly = sjly;
	}

	public String getRksj() {
		return rksj;
	}

	public void setRksj(String rksj) {
		this.rksj = rksj;
	}

	public String getIsrl() {
		return isrl;
	}

	public void setIsrl(String isrl) {
		this.isrl = isrl;
	}

	public String getRlrxm() {
		return rlrxm;
	}

	public void setRlrxm(String rlrxm) {
		this.rlrxm = rlrxm;
	}

	public String getRldwdm() {
		return rldwdm;
	}

	public void setRldwdm(String rldwdm) {
		this.rldwdm = rldwdm;
	}

	public String getRldwmc() {
		return rldwmc;
	}

	public void setRldwmc(String rldwmc) {
		this.rldwmc = rldwmc;
	}

	public String getBh() {
		return bh;
	}

	public void setBh(String bh) {
		this.bh = bh;
	}

	public String getXm() {
		return xm;
	}

	public void setXm(String xm) {
		this.xm = xm;
	}

	public String getSfzh() {
		return sfzh;
	}

	public void setSfzh(String sfzh) {
		this.sfzh = sfzh;
	}

	public String getCsrq() {
		return csrq;
	}

	public void setCsrq(String csrq) {
		this.csrq = csrq;
	}
	
	public void setCz(String cz) {
		this.cz = cz;
	}
	
	public String getCz() {
		String htmlStr = "<a href='#' onclick=\"crimeTqInfo('"+this.getBh()+"');\">提取信息</a> &nbsp;";
		htmlStr += "<a href='#' onclick=\"openCrimeDetails('"+this.getBh()+"');\">查看详情</a>";
		return htmlStr;
		
	}

	public String getEndJdsj() {
		return endJdsj;
	}

	public void setEndJdsj(String endJdsj) {
		this.endJdsj = endJdsj;
	}

	public String getStartJdsj() {
		return startJdsj;
	}

	public void setStartJdsj(String startJdsj) {
		this.startJdsj = startJdsj;
	}

	public String getJdjg() {
		return jdjg;
	}

	public void setJdjg(String jdjg) {
		this.jdjg = jdjg;
	}

	public String getJdjgdm() {
		return jdjgdm;
	}

	public void setJdjgdm(String jdjgdm) {
		this.jdjgdm = jdjgdm;
	}

	public String getRylx() {
		return rylx;
	}

	public void setRylx(String rylx) {
		this.rylx = rylx;
	}

	public String getXbmc() {
		xbmc = DictionaryCacheHellper.getSystemDictionaryValueByZdlbAndDm(CommonConstant.ZDRY_ZDLB_XB, xb);
		return xbmc;
	}

	public void setXbmc(String xbmc) {
		this.xbmc = xbmc;
	}
}
