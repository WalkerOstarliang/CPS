...
<title>社区戒毒人员</title>
		...
		<table width="98%" class="table_list09" border="0" cellSpacing="0"
			cellPadding="0" id="resultTable">
				...
				<c:forEach var="sqjd" items="${list}" varStatus="_i">
				<tr>
					<td style="text-align: center;">${sqjd.showJfFlag}</td>
					<td>${sqjd.xm}</td>
					<td>${sqjd.aj}</td>
					<td>${sqjd.zz}</td>
					<td>${sqjd.sq}</td>
					<td>${sqjd.wsh}</td>
					<td style="text-align: center;"><a href="javascript:void(0);" onclick="optRyInfo('${sqjd.ajbh}','${sqjd.rybh}');">操作</a></td>
				</tr>
				</c:forEach>
				</c:if>				
		</table>
<script type="text/javascript">
	function optRyInfo(ajbh, rybh){
		...
		var param = "?cardID=" + sfzh + "&jiaose=" + ryjs + "&jinghao=" + ryjh + "&ajbh=" + ajbh + "&rybh=" + rybh + "&rylx=3&jzcl=1"
		var url = zfbaUrl + param;
		window.open(url);
	}
	function searchData(val){
		var url = contextPath + "/xqgk/showSqjdDetail.action?searchType=" + val;
		window.location = url;
	}	
</script>		
</body>