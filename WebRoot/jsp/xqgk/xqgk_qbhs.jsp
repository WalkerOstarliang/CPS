...
<title>取保候审人员</title>
		...
		<table width="98%" class="table_list09" border="0" cellSpacing="0"
			cellPadding="0" id="resultTable">
				...
				<tr>
					<td style="text-align: center;">${qbhs.showJfFlag}</td>
					<td>${qbhs.xm}</td>
					<td>${qbhs.zm}</td>
					<td>${qbhs.yy}</td>
					<td>${qbhs.wsh}</td>
					<td>${qbhs.rq}</td>
					<td style="text-align: center;"><a href="javascript:void(0);" onclick="optRyInfo('${qbhs.ajbh}','${qbhs.rybh}');">操作</a></td>
				</tr>
				</c:forEach>
				</c:if>				
		</table>
<script type="text/javascript">
	function optRyInfo(ajbh, rybh){
		...
		var param = "?cardID=" + sfzh + "&jiaose=" + ryjs + "&jinghao=" + ryjh + "&ajbh=" + ajbh + "&rybh=" + rybh + "&rylx=1&jzcl=1"
		var url = zfbaUrl + param;
		window.open(url);
	}
	
	function searchData(val){
		var url = contextPath + "/xqgk/showQbhsDetail.action?searchType=" + val;
		window.location = url;
	}
</script>		
</body>