	    	...
	    	<td valign="top">
				<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_list07">
				  <tr>
				  	...
				  	<th width="25%" style="text-align: center;">实体建筑</th>
				  </tr>
				  
				  <c:if test="${userDeptLevel == 5}"><!-- 当前机构为警务区，显示社区数据 -->
					  <c:forEach var="syjz" items="${syjzList}" varStatus="_i">
					  <!-- 机构不再钻取，除合计、总计外数据可点击查看详细 -->
					  <tr>
					  	<td style="text-align: left;">${syjz.deptName}</td>
					   <c:if test="${_i.index != (fn:length(syjzList) - 1)}"><!-- 不是合计 -->
					  	<td style="text-align: right;"><a class="xqgk" href="javascript:void(0);" onclick="showDetail('${syjz.deptCode}', 'stjz');">${syjz.showStjz}</a></td>
					   </c:if>
					   <c:if test="${_i.index == (fn:length(syjzList) - 1)}"><!-- 是合计 -->
					  	<td style="text-align: right;">${syjz.showStjz}</td>
					   </c:if>
					  </tr>	
					  </c:forEach>				  
				  </c:if>
				  <c:if test="${userDeptLevel != 5}"><!-- 当前机构不是警务区，显示下级机构数据 -->
					  ...
					  <tr>
					   <c:if test="${_i.index != (fn:length(syjzList) - 1)}"><!-- 不是合计 -->
					  	<td style="text-align: left;"><a class="xqgk" href="###" onclick="showChildDept('${syjz.deptCode}', '${syjz.deptName}');">${syjz.deptName}</a></td>
					   </c:if>
					   <c:if test="${_i.index == (fn:length(syjzList) - 1)}"><!-- 是合计 -->
					  	<td style="text-align: left;">${syjz.deptName}</td>	
					   </c:if>
...
<script type="text/javascript">
	function showChildDept(deptCode, deptName){
		var linkUrl = contextPath + "/xqgk/getDeptInfoOfSyjz.action?deptCode=";
