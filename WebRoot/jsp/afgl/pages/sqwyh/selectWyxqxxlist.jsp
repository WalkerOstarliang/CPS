...
<creator:box>
	<creator:tbar id="tbar">
		<cps:button value="查&nbsp;询" onclick="wyxqxxQuery()"></cps:button>
	</creator:tbar>
	<creator:panel id="xldcx" title="物业小区信息列表" tbarId="tbar">
		<form id="wyxqqueryform" >
			<cps:table>
				<cps:row>
					...
					<cps:tdContent width="25%">
						<c:choose>
							<c:when test="${empty loginInfo.pcsdm}">
								<s:select id="pcsdm" list="pcsGajgjgList"  name="queryBeanSt.pcsdm" listKey="dm" listValue="jc" headerKey="" headerValue="--请选择--" cssClass="cps-select" onchange="doubleSelectJWS(this.value,'jwsselect')"></s:select>
							</c:when>
							<c:otherwise>
				...
				<cps:row>
					...
					<cps:tdLabel>物业小区名称：</cps:tdLabel>
					<cps:tdContent>
	...
	<cps:tableGrid id="wyxqquerylist" title="物业小区列表"
				mutilSelect="false"
				url="/afgl/sqldzgl/queryWyxqPageResult.action" 
				autoload="true"
				searchform="wyxqqueryform">
		<cps:column isNumber="true" title="序号"></cps:column>
		<cps:column isCheckbox="true"></cps:column>
		<cps:column field="cz" title="操作" rowcallback="czWyxqselectRowCallback"></cps:column>
		<cps:column field="xqmc" title="物业小区名称"></cps:column>
		<cps:column field="dzmc" title="物业小区地址"></cps:column>
	</cps:tableGrid>
</creator:box>
</creator:view>