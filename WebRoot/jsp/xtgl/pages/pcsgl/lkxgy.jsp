...
<creator:view width="99.8%">
	...
	<form id="hiddenform">
	<input type="hidden" id="fjlb" name="queryBean.fjlb" value="1">
	...
	<cps:tbar>
		...
		<cps:button value="新增" onclick="openFjPage('','add','1')"></cps:button>
		<cps:button value="导出" onclick="exportLkXgyInfo()"></cps:button>
	</cps:tbar>
	<cps:panel id="fwquerypanel" title="流口协管员查询" >
			<cps:table>
					<cps:row>
						...
						<cps:tdContent  width="25%">
							<c:choose>
								<c:when test="${empty loginInfo.countrygajgdm}">
									<s:select id="countrygajgjgdm" list="countryGajgjgList"  name="queryBean.countrygajgjgdm" listKey="dm" listValue="jc" headerKey="" headerValue=" " cssClass="cps-select" onchange="doublePcsGajgjgForLkXgy(this.value,'pcsdm')" ></s:select>
								</c:when>
								<c:otherwise>
				...
				<cps:row>
					...
					<cps:tdContent>
						<cps:select zdlb="ZDY_LKXGYGW" parentDm="1" id="gzgw" name="queryBean.gzgw" headerKey="" headerValue=" " value="0"></cps:select>
					</cps:tdContent>
