-- Add/modify columns 
alter table T_COMMON_PCSJBQKTJB add SQJWSPBZFBZFJS number;
alter table T_COMMON_PCSJBQKTJB add SQJWSPBLKXGYS number;
alter table T_COMMON_PCSJBQKTJB add SQJWSPBXFDYS number;
-- Add comments to the columns 
comment on column T_COMMON_PCSJBQKTJB.SQJWSPBZFBZFJS
  is '社区(驻村)警务室配备政法编制辅警数';
comment on column T_COMMON_PCSJBQKTJB.SQJWSPBLKXGYS
  is '社区(驻村)警务室配备流动人口与出租屋协管员数';
comment on column T_COMMON_PCSJBQKTJB.SQJWSPBXFDYS
  is '社区(驻村)警务室配备巡防队员数';
                  ...
                  --社区(驻村)警务室配备流动人口与出租屋协管员数
                   ...
                   INSERT INTO T_COMMON_PCSJBQKTJB
                    (ID, ORGCODE, ORGNAME, LEVE,PARENTCODE, ZJLS, PCSZS, 
                         ...
                         JWSLGANWS,STPZS,PCSZFBZFJS,PCSLDRKXGYS,PCSXFDYS,SQJWSPBZFBZFJS,SQJWSPBXFDYS,SQJWSPBLKXGYS)
                   VALUES
                    (I, LIST(I).ORGCODE, LIST(I).ORGNAME, LIST(I).LEVE,LIST(I).PARENT_CODE, V_ZJLS, V_PCSZS, 
                        ...
                        V_JWSLGANWS,V_STPZS,V_PCSZFBZFJS,V_PCSLDRKXGYS,V_PCSXFDYS,V_SQJWSPBZFBZFJS,V_SQJWSPBXFDYS,V_SQJWSPBLKXGYS);
                   COMMIT;
