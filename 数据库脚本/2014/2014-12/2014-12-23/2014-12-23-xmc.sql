...
AS
       ...
       --居住证统计报表
       PROCEDURE P_TJBB_JZZTJBB(V_TJQSSJ DATE, V_TJJZSJ DATE);
...
CREATE OR REPLACE PACKAGE BODY SQJW_TJBB_RK IS
       ...
       PROCEDURE P_TJBB_JZZTJBB(V_TJQSSJ DATE, V_TJJZSJ DATE)
       AS
            CURSOR ORG_JWT_LIST IS
                 SELECT ORGCODE, ORGNAME, PARENT_CODE, LEVE
                 FROM T_COMMON_ORGANIZATION_SJJS
                 WHERE LEVE=5;  
           
            CURSOR ORG_PUP_LIST IS
                 SELECT ORGCODE, ORGNAME, PARENT_CODE, LEVE
                 FROM T_COMMON_ORGANIZATION_SJJS
                 WHERE LEVE<5;    
           
           TYPE  TYPE_ORG_LIST IS TABLE OF ORG_JWT_LIST%ROWTYPE;
           LIST   TYPE_ORG_LIST;
       ...
       BEGIN
            ...
            OPEN ORG_JWT_LIST;
            LOOP
                 FETCH ORG_JWT_LIST BULK COLLECT INTO LIST LIMIT 1000;
                 EXIT WHEN LIST.COUNT=0;
                 FOR I IN LIST.FIRST .. LIST.LAST LOOP
                      --流动人口数
                      SELECT COUNT(1) INTO V_LDRKZS
                      ...
                      --总办证数
                      SELECT COUNT(1) INTO V_ZBZS
                      FROM T_LSGL_LDRK_JZZXX 
                      WHERE DJDW=LIST(I).ORGCODE
                            AND ZZSJ>=V_TJQSSJ 
                            AND ZZSJ<V_TJJZSJ
                            AND JZZDYZT='20' 
                            AND DYZT='1';
                      ...
                      --本省办证人数      
                      SELECT COUNT(1) INTO V_BSBZRS
                      FROM T_LSGL_LDRK_JZZXX JZ INNER JOIN T_LSGL_RK_JBXX RK ON RK.RYBH=JZ.JZZRYBH 
                      WHERE DJDW=LIST(I).ORGCODE
                            AND ZZSJ>=V_TJQSSJ
                            AND ZZSJ<V_TJJZSJ
                            AND JZZDYZT='20' 
                            AND DYZT='1'
                            AND RK.HJDQH LIKE '43%';
                     
                     --外省办证人数  
                     SELECT COUNT(1)  INTO V_WSBZRS
                     FROM T_LSGL_LDRK_JZZXX JZ INNER JOIN T_LSGL_RK_JBXX RK ON RK.RYBH=JZ.JZZRYBH 
                     WHERE DJDW=LIST(I).ORGCODE
                          AND ZZSJ>=V_TJQSSJ
                          AND ZZSJ<V_TJJZSJ
                          AND JZZDYZT='20' 
                          AND DYZT='1'
                          AND RK.HJDQH NOT LIKE '43%';
                     
                     --男办证人数
                     SELECT COUNT(1) INTO V_NANBZRS
                     FROM T_LSGL_LDRK_JZZXX JZ
                     WHERE DJDW=LIST(I).ORGCODE 
                          AND ZZSJ>=V_TJQSSJ
                          AND ZZSJ<V_TJJZSJ
                          AND JZZDYZT='20' 
                          AND DYZT='1'
                          AND DECODE(SUBSTR(JZ.SFZH,17,1),'1','1','3','1','5','1','7','1','9','1','2')='1';    
                     
                     --女办证人数
                     SELECT COUNT(1) INTO V_NVBZRS
                     FROM T_LSGL_LDRK_JZZXX JZ
                     WHERE DJDW=LIST(I).ORGCODE 
                          AND ZZSJ>=V_TJQSSJ
                          AND ZZSJ<V_TJJZSJ
                          AND JZZDYZT='20' 
                          AND DYZT='1'
                          AND DECODE(SUBSTR(JZ.SFZH,17,1),'1','1','3','1','5','1','7','1','9','1','2')='2';    
                      
                     --废证数
                    SELECT COUNT(1) INTO V_FZS
                    FROM T_LSGL_LDRK_JZZXX JZ
                    WHERE DJDW=LIST(I).ORGCODE 
                          AND ZZSJ>=V_TJQSSJ 
                          AND ZZSJ<V_TJJZSJ
                    ...
                    SELECT COUNT(1) INTO V_COUNT FROM T_TJFX_JZZTJBB WHERE  ORGCODE=LIST(I).ORGCODE AND TJRQ=V_TJRQ;
                    
                    IF V_COUNT=0 THEN
                          INSERT INTO T_TJFX_JZZTJBB
                            (ORGCODE, ORGNAME, PARENT_CODE, LEVE, TJRQ, 
                             TJSJ, TJQSSJ, TJJZSJ, LDRKZS, ZBZS, BSBZRS, 
                             WSBZRS, NANBZRS, NVBZRS, FZS, YSBLS)
                          VALUES
                            (LIST(I).ORGCODE, LIST(I).ORGNAME, LIST(I).PARENT_CODE, LIST(I).LEVE, V_TJRQ, 
                            V_TJSJ, V_TJQSSJ, V_TJJZSJ, V_LDRKZS, V_ZBZS, V_BSBZRS, 
                            V_WSBZRS, V_NANBZRS, V_NVBZRS, V_FZS, V_YSBLS);
                          
                    
                    ELSE
                          UPDATE T_TJFX_JZZTJBB
            ...
            OPEN ORG_PUP_LIST;
            LOOP
                 FETCH ORG_PUP_LIST BULK COLLECT INTO LIST LIMIT 1000;
                 EXIT WHEN LIST.COUNT=0;
                 FOR I IN LIST.FIRST .. LIST.LAST LOOP
                    SELECT SUM(LDRKZS), SUM(ZBZS), SUM(BSBZRS), SUM(WSBZRS), 
                           SUM(NANBZRS), SUM(NVBZRS), SUM(FZS), SUM(YSBLS)
                           INTO V_LDRKZS,V_ZBZS,V_BSBZRS,V_WSBZRS,V_NANBZRS,V_NVBZRS,V_FZS,V_YSBLS
                    FROM T_TJFX_JZZTJBB
...
CREATE OR REPLACE PROCEDURE P_EXEC_TJBB_JZZTJBB(V_TYPE VARCHAR2)
AS
     V_CURRENTDATE  DATE;
BEGIN
     --初始化
     IF V_TYPE = '1' THEN
         V_CURRENTDATE:=TO_DATE('20140901','YYYYMMDD');
         LOOP
               V_CURRENTDATE := V_CURRENTDATE + 1;
               EXIT WHEN V_CURRENTDATE>=trunc(SYSDATE);
               SQJW_TJBB_RK.P_TJBB_JZZTJBB(V_CURRENTDATE,V_CURRENTDATE + 1);
         END LOOP;
     --每天计算
     ELSIF V_TYPE = '2' THEN
