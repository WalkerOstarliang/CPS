package com.chinacreator.lsgl.bean;

import com.chinacreator.common.bean.BaseValue;

/**
 * 社区康复
 */
public class ZdrySqkfBean extends BaseValue {

	private static final long serialVersionUID = 3885731525754193900L;

	/**
	 * 性别
	 */
	private String xingbie;
	
	/**
	 * 工作单位
	 */
	private String gongzuodanwei;
	
	/**
	 * 现住地址
	 */
	private String zhuzhidizhi;
	
	/**
	 * 年龄
	 */
	private String nianling;
	
	/**
	 * 省份证号
	 */
	private String zhenjianhaoma;
	
	/**
	 * 出生日期
	 */
	private String chushengriqi;
	
	/**
	 * 案件名称
	 */
	private String anjianmingcheng;
	
	/**
	 * 案件编号
	 */
	private String anjianbianhao;
	
	/**
	 * 诉讼机关
	 */
	private String susongjiguan;
	
	/**
	 * 服役机关
	 */
	private String fuyijiguan;
	
	/**
	 * 法律依据
	 */
	private String zelingyiju;
	
	/**
	 * 违法事实
	 */
	private String weifashishi;
	
	/**
	 * 康复期限
	 */
	private String kangfuqixian;
	
	/**
	 * 起始时间
	 */
	private String qishishijian;
	
	/**
	 * 终止时间
	 */
	private String zhongzhishijian;
	
	/**
	 * 执行社区
	 */
	private String zhixingshequ;
	
	/**
	 * 社区地址
	 */
	private String shequdizhi;
	
	/**
	 * 文书文号
	 */
	private String wenshuwenhao;
	

	public String getGongzuodanwei() {
		return gongzuodanwei;
	}

	public void setGongzuodanwei(String gongzuodanwei) {
		this.gongzuodanwei = gongzuodanwei;
	}

	public String getZhuzhidizhi() {
		return zhuzhidizhi;
	}

	public void setZhuzhidizhi(String zhuzhidizhi) {
		this.zhuzhidizhi = zhuzhidizhi;
	}

	public String getNianling() {
		return nianling;
	}

	public void setNianling(String nianling) {
		this.nianling = nianling;
	}

	public String getZhenjianhaoma() {
		return zhenjianhaoma;
	}

	public void setZhenjianhaoma(String zhenjianhaoma) {
		this.zhenjianhaoma = zhenjianhaoma;
	}

	public String getChushengriqi() {
		return chushengriqi;
	}

	public void setChushengriqi(String chushengriqi) {
		this.chushengriqi = chushengriqi;
	}

	public String getAnjianmingcheng() {
		return anjianmingcheng;
	}

	public void setAnjianmingcheng(String anjianmingcheng) {
		this.anjianmingcheng = anjianmingcheng;
	}

	public String getAnjianbianhao() {
		return anjianbianhao;
	}

	public void setAnjianbianhao(String anjianbianhao) {
		this.anjianbianhao = anjianbianhao;
	}

	public String getSusongjiguan() {
		return susongjiguan;
	}

	public void setSusongjiguan(String susongjiguan) {
		this.susongjiguan = susongjiguan;
	}

	public String getFuyijiguan() {
		return fuyijiguan;
	}

	public void setFuyijiguan(String fuyijiguan) {
		this.fuyijiguan = fuyijiguan;
	}

	public String getZelingyiju() {
		return zelingyiju;
	}

	public void setZelingyiju(String zelingyiju) {
		this.zelingyiju = zelingyiju;
	}

	public String getWeifashishi() {
		return weifashishi;
	}

	public void setWeifashishi(String weifashishi) {
		this.weifashishi = weifashishi;
	}

	public String getKangfuqixian() {
		return kangfuqixian;
	}

	public void setKangfuqixian(String kangfuqixian) {
		this.kangfuqixian = kangfuqixian;
	}

	public String getQishishijian() {
		return qishishijian;
	}

	public void setQishishijian(String qishishijian) {
		this.qishishijian = qishishijian;
	}

	public String getZhongzhishijian() {
		return zhongzhishijian;
	}

	public void setZhongzhishijian(String zhongzhishijian) {
		this.zhongzhishijian = zhongzhishijian;
	}

	public String getZhixingshequ() {
		return zhixingshequ;
	}

	public void setZhixingshequ(String zhixingshequ) {
		this.zhixingshequ = zhixingshequ;
	}

	public String getShequdizhi() {
		return shequdizhi;
	}

	public void setShequdizhi(String shequdizhi) {
		this.shequdizhi = shequdizhi;
	}

	public String getXingbie() {
		return xingbie;
	}

	public void setXingbie(String xingbie) {
		this.xingbie = xingbie;
	}

	public String getWenshuwenhao() {
		return wenshuwenhao;
	}

	public void setWenshuwenhao(String wenshuwenhao) {
		this.wenshuwenhao = wenshuwenhao;
	}
}
